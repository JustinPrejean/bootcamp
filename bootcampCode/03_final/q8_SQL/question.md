...
SELECT players.team_id, teams.name, teams.city, teams.state, concat(fname, ' ', lname) AS fullname, weight,
CASE 
    WHEN weight > 200 THEN 'Heavy'
    ELSE 'Medium'
    END AS  WeightCategory
FROM hr.players
RIGHT JOIN teams on team_id = teams.id
ORDER BY team_id, weight DESC 